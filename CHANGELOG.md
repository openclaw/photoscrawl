...
- Add `people`, `find`, `rank`, and `junk` over the imported signals: required-people, date, place, and text filters; best-first ranking with reasons that replaces missing quality signals with their median, grouped by burst, time gap, or day; and screenshot, blurry, eyes-closed, and duplicate candidates with a suggested keeper.
- Add `sheet` for numbered contact sheets rendered from local files without PhotoKit, `similar` for label-overlap matches outside the seed's own moment that exclude hidden and other ineligible assets before shortlisting, `forgotten` for strong photos in no user or shared album, and `share-check`, a privacy gate that blocks sensitive categories with per-category negation and fails closed on unclassified photos. Schema 5 adds a visual-label index.
